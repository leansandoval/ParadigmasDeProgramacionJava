...
public class AtraccionIteratorImpl implements AtraccionIterator {
	...
	/*
	 * @Override public Atraccion next() { List<Atraccion> atraccionesList = new
	 * ArrayList<>(atracciones.values()); Atraccion atraccion = null; while
	 * (posicion < atraccionesList.size()) { atraccion =
	 * atraccionesList.get(posicion); if (atraccion.getCupoDiario() == 0 ||
	 * atraccion.getPrecio() > usuario.getPresupuesto() || atraccion.getTiempo() >
	 * usuario.getTiempoDisponible() || !atraccion.getDisponibilidad()) {
	 * posicion++; } else { posicion++; return atraccion; } } return null; }
	 */
